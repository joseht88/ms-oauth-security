node {
   ...
   stage('Preparation') {
       git 'https://github.com/joseht88/ms-oauth-security.git'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/ms-oauth-security/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/ms-oauth-security/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/ms-oauth-security/runDeployment.sh'
       }catch (e){
